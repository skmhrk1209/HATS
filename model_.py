...
class Model(object):
    ...
    def __call__(self, features, labels, mode):
        ...
        attention_maps = map_innermost_list(
            function=lambda attention_maps: tf.add_n(attention_maps),
            sequence=attention_maps
        )

        bounding_boxes = map_innermost_element(
            function=compose(
                lambda attention_maps: tf.layers.flatten(
                    inputs=attention_maps
                ),
                lambda attention_maps: tf.layers.dense(
                    inputs=attention_maps,
                    units=4
                )
            ),
        ...
        while all(flatten_innermost_element(map_innermost_element(lambda labels: len(labels.shape) > 1, labels["class"]))):

            labels["class"] = map_innermost_element(
                function=lambda labels: tf.unstack(labels, axis=1),
                sequence=labels["class"]
            )

        while all(flatten_innermost_element(map_innermost_element(lambda labels: len(labels.shape) > 2, labels["box"]))):

            labels["box"] = map_innermost_element(
                function=lambda labels: tf.unstack(labels, axis=1),
                sequence=labels["box"]
            )
        ...
        loss = tf.reduce_mean(map_innermost_element(
            ...
            sequence=zip_innermost_element(logits, labels["class"])
        ))
        ...
        loss += tf.reduce_mean(map_innermost_element(
            function=lambda boxes_labels: tf.losses.huber_loss(
                predictions=boxes_labels[0],
                labels=boxes_labels[1],
                reduction=tf.losses.Reduction.NONE
            ),
            sequence=zip_innermost_element(bounding_boxes, labels["box"])
        )) * self.hyper_params.box_regression_decay
