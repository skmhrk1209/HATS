...
def main(unused_argv):
    ...
    classifier = tf.estimator.Estimator(
        ...
        )(features["image"] if isinstance(features, dict) else features, labels, mode),
    ...
    if args.predict:
        ...
        predict_results = classifier.predict(
            input_fn=lambda: tf.data.Dataset.from_tensor_slices(images).map(
                lambda images: images, None).batch(args.batch_size).make_one_shot_iterator().get_next()
        )
