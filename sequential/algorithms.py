...
def enumerate_innermost(sequence, predicate, indices=()):

    return (indices, sequence) if predicate(sequence) else type(sequence)(map(lambda index_element: enumerate_innermost(index_element[1], predicate, indices + (index_element[0],)), enumerate(sequence)))
