...
int main(int argc, char *argv[]) {
    ...
    options_description.add_options()("input_directory", boost::program_options::value<std::string>(), "directory of input data")(
        ...
        "num_words", boost::program_options::value<int>()->default_value(5), "maximum number of words contained in a instance")(
        "num_chars", boost::program_options::value<int>()->default_value(10), "maximum number of characters contained in a instance")(
        "num_instances", boost::program_options::value<int>()->default_value(900000), "number of instances that will be generated")(
        "num_retries", boost::program_options::value<int>()->default_value(100), "maximum number of retries for locating bounding box");
    ...
    std::mutex mutex;
    boost::progress_display progress_display(variables_map["num_instances"].as<int>());

    std::vector<std::thread> threads;
    auto num_threads = std::thread::hardware_concurrency();
    auto num_instances = variables_map["num_instances"].as<int>() / num_threads;

    for (auto i = 0; i < num_threads; ++i) {
        threads.emplace_back([&, i]() {
            for (auto j = num_instances * i; j < num_instances * (i + 1); ++j) {
                ...
                auto num_words = std::uniform_int_distribution<int>(1, variables_map["num_words"].as<int>())(engine);
                while (words.size() < num_words) {
                    if (![&]() {
                            for (auto k = 0; k < variables_map["num_retries"].as<int>(); ++k) {
                                ...
                                if (match.str(1).size() > variables_map["num_chars"].as<int>()) {
                                    continue;
                                }
                                ...
                                if ([&]() {
                                        for (auto l = 0; l < variables_map["num_retries"].as<int>(); ++l) {
                                            ...
                                            if (boost::algorithm::all_of(words, [&](const auto &word) { return boost::geometry::disjoint(word.second, box); })) {
                ...
                boost::sort(words, [](const auto &word1, const auto &word2) {
                    return (word1.second.min_corner().y() < word2.second.min_corner().y()) ||
                           ((word1.second.min_corner().y() == word2.second.min_corner().y()) && (word1.second.min_corner().x() < word2.second.min_corner().x()));
                });
